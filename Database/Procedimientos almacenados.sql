...
delimiter //
create procedure p_searchUsers (in search varchar(255), in codigo_empresa int)
begin
  Select usuarios.codigo, tipo_usuarios.tipo_usuario, usuarios.nombre_usuario, 
  usuarios.nombre, telefonos.telefono, usuarios.email, 
  CASE WHEN usuarios.estado = 1 Then 'Activo' ELSE 'Inactivo' END AS estado
...
/*Obtener todos los usuarios del sistema según una empresa, estado (Activo, inactivo o todos)*/
delimiter //
create procedure p_getUserByStatus (in codigo_empresa int, in search varchar(25))
begin
  ...
  if search = 'Activos' or search = 'Inactivos' then 
      ...
      FROM usuarios join tipo_usuarios on usuarios.codigo_tipo_usuario = tipo_usuarios.codigo
	  join telefonos on usuarios.codigo_telefono = telefonos.codigo
      join  empresas on usuarios.codigo_empresa = empresas.codigo where empresas.codigo = codigo_empresa and usuarios.estado = (select @estado);
      
  /*Buscar todos los usuarios (Activos e inactivos)*/
  else 
    call p_getUsers(codigo_empresa);
  end if;
end
//