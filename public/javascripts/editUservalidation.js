...
const sameUsername = document.getElementById('usernameText');
...
//Por defecto, están en verdadero, es decir, correcto. 
//Esto debido a que son datos que se van a editar
const campos = {
  name: true,
  username: true,
  telefono: true,
...
const validarUsuarioRepetido = async () =>{
  ...
  try {
    const respuesta  = await fetch('/users/getUser?user='+username.value+'');
    const usuario = await respuesta.json();

    //Verifica si el nombre de usuario ya existe
    //Tambien comprueba que sea un nombre de usuario distinto al actual
    if(usuario.length > 0 && (sameUsername.textContent != usuario[0]['nombre_usuario'])){
...
formulario.addEventListener('submit', (event) => {
  ...
  if (campos.username && campos.telefono && campos.name && campos.email) {
